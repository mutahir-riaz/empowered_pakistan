...
const AdminPage = () => {
  ...
  // const { toast } = useToast(); // <-- 2. Remove the old hook
  ...
  const handleCreateGalleryItem = async (formData: FormData) => {
    try {
      const newGalleryItem: Gallery = {
        id: Math.max(...gallery.map((item) => item.id), 0) + 1,
        title: formData.get("title") as string,
        description: formData.get("description") as string,
      ...
      setGallery((prev) => [...prev, newGalleryItem]);
  ...
  const handleUpdateGalleryItem = async (formData: FormData) => {
    ...
    try {
      ...
      setGallery((prev) =>
        prev.map((item) =>
          item.id === editingGalleryItem.id ? updatedGalleryItem : item
        )
      );
          ...
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <DialogContent className="max-w-2xl">
                ...
                <form
                  action={editingEvent ? handleUpdateEvent : handleCreateEvent}
                >
